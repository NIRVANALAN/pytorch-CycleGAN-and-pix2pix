...
def load_colmap_pose(realdir, pose=False):
    ...
    names = [realdir / "images" / imdata[k].name for k in imdata]
...
def load_colmap_data(realdir):
    ...
    poses = np.concatenate(
        [poses, np.tile(hwf[..., np.newaxis], [1, 1, poses.shape[-1]])], 1  # TODO
    )
...
def load_poses(basedir, match_type, factors=None):
    ...
    if os.path.exists(os.path.join(basedir, "sparse/0")):
        ...
        files_had = []
    assert all([f in files_had for f in files_needed])
    print("Post-colmap")
