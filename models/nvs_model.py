...
class NVSModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm="batch", netG="dcgan", dataset_mode="aligned")
        if is_train:
            ...
            parser.add_argument(
                ...
                type=bool,
                default=False,
                help="whether to impose content perceptual(VGG) Loss in net_D",
            )
            parser.add_argument(
                "--lambda_vgg",
                type=float,
                default=10.0,
                help="lambda weight of perceptual loss",
            )
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            if opt.vgg_loss:
                self.criterion_content = networks.PerceptualLoss()
    ...
    def backward_D(self):
        ...
        # net_D is trained to descriminate: 1. if B is realistic enough 2. if A and B match each other
    ...
    def backward_G(self):
        ...
        # Second, G(A) = B
        self.loss_G_L1 = (
            self.criterionL1(self.fake, self.gt) * self.opt.lambda_L1
        )  # 100 by default
        # combine loss and calculate gradients
        ...
        if self.opt.vgg_loss:
            self.loss_Content = (
                self.criterion_content(self.fake, self.gt) * self.opt.lambda_vgg
            )
